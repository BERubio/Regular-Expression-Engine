...
let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t =
  ...
  | Char(a) -> let state1 = fresh() in let state2 = fresh()
    in {qs = [state1; state2]; sigma = [a]; q0 =state1; fs = [state2]; delta = [(state1, Some a, state2)]}
  ...
  |Union(a,b) -> let nfa1 = regexp_to_nfa a in
   ...
   let state1 = fresh() in 
   let state2 = fresh() in {qs = (union (union nfa1.qs nfa2.qs) [state1; state2]); 
     sigma = union nfa1.sigma nfa2.sigma; q0 = state1; fs = [state2];
     delta = union (union nfa1.delta nfa2.delta) [(state1, None, nfa1.q0); (state1, None, nfa2.q0);
     ((match nfa1.fs with | [] -> 0 | h::t -> h), None, state2); ((match nfa2.fs with | [] -> 0 | h::t -> h), None, state2)]    }
  | Star(a) -> let nfa1 = regexp_to_nfa a in
     let state1 = fresh() in
     let state2 = fresh() in
     {qs = (union (nfa1.qs) [state1; state2]); sigma = nfa1.sigma; q0 = state1; fs = [state2];
      delta = union (nfa1.delta) [((match nfa1.fs with | [] -> 0 | h::t -> h), None, state2); (state1, None, nfa1.q0);
      (state1, None, state2); (state2, None, state1)]}
